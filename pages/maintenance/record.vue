<template>
  <view class="page">
    ...
    <view v-else-if="list.length === 0" class="empty-wrap">
      ...
      <u-button text="去查询" type="primary" color="#57ca9e" @click="uni.navigateTo({ url: '/pages/maintenance/vinInput' })" />
    </view>
    <scroll-view v-else scroll-y class="list-wrap">
      <view class="record-item" v-for="(item, index) in list" :key="index" @click="goReport(item)">
...
async function fetchList() {
  loading.value = true
  try {
    const res = await getCheckList({ type: 3 })
...
function goReport(item) {
  ...
  if (id) {
    uni.navigateTo({ url: `/pages/maintenance/report?checkId=${id}&payOrderId=${item.payOrderId || ''}` })
  }
}
</script>
...
.empty-wrap { display: flex; flex-direction: column; align-items: center; padding: 150rpx 40rpx 0; gap: 30rpx;
  .empty-img { width: 160rpx; height: 200rpx; } .empty-text { font-size: 30rpx; color: #888; } }
