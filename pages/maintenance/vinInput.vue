  ...
  <view class="page">
    ...
    <view class="top-bg-wrap">
      ...
      <view class="top-content">
        <view class="top-text">
          <text class="top-title">维保查询</text>
          <text class="top-desc">查询车辆维修保养历史记录，全面掌握车辆使用状况。</text>
        </view>
        <image src="/static/top-icon3.png" class="top-icon" mode="aspectFit" />
      </view>
    </view>
...
import { payCloudCheck, carWbCreatedReport } from '@/utils/api'
...
async function handleSubmit() {
  ...
  loading.value = true
  try {
    const payRes = await payCloudCheck({ payType: 5, vinCode: vinCode.value })
    if (payRes.code !== 200) {
    ...
    const res = await carWbCreatedReport({ vinCode: vinCode.value, outTradeNo, vinImg: vinImgPath.value || '' })
    uni.hideLoading()

    if (res.code === 200) {
      uni.navigateTo({ url: `/pages/maintenance/report?checkId=${res.data.checkId}&payOrderId=${res.data.payOrderId}` })
    } else {
