<template>
  <view class="page">
    ...
    <view v-else-if="list.length === 0" class="empty-wrap">
      ...
      <u-button text="去查询" type="primary" color="#30ad55" size="small" @click="uni.navigateTo({ url: '/pages/accident/vinInput' })" />
    </view>
...
async function fetchList() {
  loading.value = true
  try {
    const res = await getCheckList({ type: 2, pageNum: 1, pageSize: 100 })
...
function statusLabel(item) {
  if (item.status === 0 && item.payStatus >= 2) return '待检测'
  const map = { 0: '待支付', 1: '生成中', 2: '已完成', 3: '已退款', 4: '已取消', 5: '联系客服' }
  return map[item.status] ?? '未知'
}

function statusClass(item) {
  if (item.status === 0 && item.payStatus >= 2) return 'status-paid'
  if (item.status === 2) return 'status-success'
  if (item.status === 3) return 'status-refund'
  if (item.status === 4) return 'status-cancel'
  if (item.status === 5) return 'status-refund'
